...
def update_statistic(env,
    ...
    if ((nearest_hour != 0) and
        ...
        total_released = {
            icu_type: hospital.daily_released_total[icu_type] for icu_type in hospital.departments}
        total_refused = {
            icu_type: hospital.daily_refused_total[icu_type] for icu_type in hospital.departments}
        total_died = {
            standard_name: hospital.daily_died_total[standard_name],
...
def update_icu_departments(env,
    ...
    if ((nearest_hour != 0) and (nearest_hour % update_frequency_in_hours == 0)):
        ...
        standard_icu_list = hospital.departments_capacity[
            standard_name]['icu_list']
        hospital.departments_capacity[
            standard_name]['icu_list'] = filter_icu_list_by_stay_duration(
                standard_icu_list,
                env.now)
        ...
        ventilated_icu_list = hospital.departments_capacity[
            ventilated_name]['icu_list']
        hospital.departments_capacity[
            ventilated_name]['icu_list'] = filter_icu_list_by_stay_duration(
                ventilated_icu_list,
                env.now)
...
def simulate(params_dictionary: dict = {
    ...
    departments_capacity = {
        ICU_Types.standard_icu.name: {
            'capacity': standard_icu_capacity,
            'icu_list': generate_random_icu_list(
                starting_standard_icu_count,
                standard_icu_stay_duration,
                ICU_Types.standard_icu.name,
                hours_in_day)},
        ICU_Types.ventilated_icu.name: {
            'capacity': ventilated_icu_capacity,
            'icu_list': generate_random_icu_list(
                starting_ventilated_icu_count,
                ventilated_icu_stay_duration,
                ICU_Types.ventilated_icu.name,
                hours_in_day)}}
    icu_properties = {
        ICU_Types.standard_icu.name: {
            'fatality_rate': standard_icu_fatality_rate,
            'stay_duration': standard_icu_stay_duration},
        ICU_Types.ventilated_icu.name: {
            'fatality_rate': ventilated_icu_fatality_rate,
            'stay_duration': ventilated_icu_stay_duration}}
    daily_released_total = {
        icu_type: 0 for icu_type in departments}
    daily_refused_total = {
        icu_type: 0 for icu_type in departments}
    ...
    env.process(patients_arrivals(env,
                                  hospital,
                                  hours_in_day,
                                  patients_amount,
                                  doubles_in_days,
